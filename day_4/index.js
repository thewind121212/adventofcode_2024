...
//soltuion get all line in matrix and check xmas in all line
...
// another way to do solution 1 backtracking
const allLines = [];
...
function checkXmasToEast(row, col) {
  if (!gotMatch(row, col + 1, "M")) {
    return;
  }
  if (!gotMatch(row, col + 2, "A")) {
    return;
  }
  if (!gotMatch(row, col + 3, "S")) {
    return;
  }
  xmasCount += 1;
}
...
function checkXmasToNorth(row, col) {
  if (!gotMatch(row - 1, col, "M")) {
    return;
  }
  if (!gotMatch(row - 2, col, "A")) {
    return;
  }
  if (!gotMatch(row - 3, col, "S")) {
    return;
  }
  xmasCount += 1;
}
...
function checkXmasToNorthEast(row, col) {
  if (!gotMatch(row - 1, col + 1, "M")) {
    return;
  }
  if (!gotMatch(row - 2, col + 2, "A")) {
    return;
  }
  if (!gotMatch(row - 3, col + 3, "S")) {
    return;
  }
  xmasCount += 1;
}
